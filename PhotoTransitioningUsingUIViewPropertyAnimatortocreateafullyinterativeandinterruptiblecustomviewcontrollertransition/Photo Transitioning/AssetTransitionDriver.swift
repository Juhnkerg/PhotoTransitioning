...
class AssetTransitionDriver: NSObject {
    ...
    init(operation: UINavigationController.Operation, context: UIViewControllerContextTransitioning, panGestureRecognizer panGesture: UIPanGestureRecognizer) {
        ...
        self.items = fromAssetTransitioning.itemsForTransition(context: context).filter({ (item) -> Bool in
            ...
            item.imageView = {
                ...
                // - 这个 longPressGestureRecognizer 会导致在fromViewController中多次点击imageView卡住?
                // longPressGestureRecognizer 的作用应该是演示动画进度用的，看 press 方法里有 pauseAnimation()
                // 但是去掉后不能在开始动画后，点击，反转transition
                // 还必须的是 UILongPressGestureRecognizer。为什么UTapGestureRecognizer无效？
    ...
    func animate(_ toPosition: UIViewAnimatingPosition) {
        ...
        // - 自己加的。默认的是false，好像不要加，是为了防止在动画中点击 击中动画终止的位置。
        /// 好像应该在 view 中重写 hitTest。那如果是 ViewController 呢？
        /**
         override func hitTest(_ point: GPoint, with event: UIEvent!) -> UIView? {
         ...
         */
//        itemFrameAnimator.isManualHitTestingEnabled = true
